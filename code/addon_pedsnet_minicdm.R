...
.measurement_cleanup <- function(m_tbl) {
  m_tbl %>%
    mutate(
      value_source_value =
        case_when(
          value_source_value %like% "%measurement: %" ~
            as.character(""),
...
personal_tables <- function() {
  tribble(
    ~tbl_expr, ~cleanup_name, ~indexes, ~output_name,
...
impersonal_tables <- function() {
  tribble(
    ~tbl_expr, ~cleanup_name, ~indexes, ~output_name, ~join_on,
  ...
  for (tabl in tables) {
    ...
    if (!is.null(dbi_con(tabl)) && length(cids) > 2) {
      ...
      tabl <- tabl %>%
        select(any_of(cids)) %>%
        distinct() %>%
        compute_new(name = tname, temporary = TRUE)
    }
    for (col in cids) {
...
build_mini_cdm <-
  function(cohort, clean = TRUE,
    ...
    t_personal <- get_personal_tables(
      cohort = cohort,
      metadata = personal,
      clean = clean
    )
    ...
    t_impersonal <- get_impersonal_tables(
      personal_tables = t_personal,
      metadata = impersonal,
      clean = clean
    )
    ...
    maybe_more <-
      reduce(map(names(t_impersonal),
        .f = function(x) {
          intersect(
            tbl_vars(t_impersonal[[x]]),
            pull(
              impersonal %>% filter(output_name != x),
              "join_on"
            )
          )
        }
    ...
    while (length(maybe_more) > 0) {
      ...
      trial <- get_impersonal_tables(
        personal_tables = t_impersonal,
    ...
    if (materialize) {
      concepts <- materialize_tbls(
        setNames(concepts, concept_out),
        tibble(
          output_name = concept_out,
          indexes = list("concept_id")
        )
      )
    }
